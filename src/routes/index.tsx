...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "THE CALL — Thriller narratif interactif" },
      {
        name: "description",
        content:
          "Une femme kidnappée vous appelle par erreur. Resterez-vous en ligne ? Jeu narratif immersif en temps réel.",
      },
      { property: "og:title", content: "THE CALL — Thriller narratif" },
      {
        property: "og:description",
        content: "Décrochez. Ne raccrochez pas. Sa vie dépend de vous.",
      },
    ],
  }),
  component: TheCallGame,
});
      ...
      <header className="border-b border-border bg-card/80 backdrop-blur">
        <div className="mx-auto flex max-w-2xl items-center justify-between px-4 py-2 text-xs font-mono">
          ...
          <div className="flex items-center gap-3">
            ...
            <Stat
              label="LIEN"
              value={world.claireConfiance}
              color="text-claire"
            />
          </div>
        </div>
        <div className="mx-auto max-w-2xl px-4 pb-2 text-[10px] uppercase tracking-widest text-muted-foreground">
          {scene.title} · {world.missionStatus === "active" ? "Mission en cours" : world.missionStatus === "failed" ? "Mission échouée" : "Mission terminée"}
        </div>
      </header>
  ...
  return (
    <div className="flex min-h-screen items-center justify-center px-6">
      <div className="max-w-md text-center">
        <div className="mb-2 font-mono text-xs uppercase tracking-[0.4em] text-danger">
          Thriller interactif
        </div>
        <h1 className="mb-4 text-6xl font-bold tracking-tight">
          THE <span className="text-primary">CALL</span>
        </h1>
        <p className="mb-8 text-sm leading-relaxed text-muted-foreground">
          Une femme appelle votre numéro par erreur. Elle dit avoir été
          kidnappée. La ligne ne doit pas tomber.
          <br />
          <br />
          Vos choix décident de son sort.
        </p>
        <button
          onClick={onStart}
          className="pulse-danger w-full rounded-md bg-primary px-6 py-4 text-lg font-semibold text-primary-foreground transition hover:opacity-90"
        >
          📞 Décrocher
        </button>
        <p className="mt-6 font-mono text-[10px] uppercase tracking-widest text-muted-foreground">
          Acte 1 — L'appel · Casque recommandé
        </p>
      </div>
    </div>
  );
